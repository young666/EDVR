...
class FaceDataset(data.Dataset):
    """
    Reading the training FaceDataset dataset \\
    Key example: X_Y.Z \\
    X: Total number of images \\
    Y: Directory \\
    Z: Image index \\
    GT: Ground-Truth; \\
    LQ: Low-Quality, e.g., low-resolution/blurry/noisy/compressed frames
    support reading N LQ frames \\
    N = 1, 3, 5, 7
    """
    ...
    def __getitem__(self, index):
        ...
        if self.opt["border_mode"]:
            ...
            if direction == 1:
                neighbor_list = list(
                    range(
                        center_frame_idx,
                        center_frame_idx + interval * N_frames,
                        interval,
                    )
                )
            else:
            ...
            name_b = "{}".format(neighbor_list[self.half_N_frames])
        ...
        for v in neighbor_list:
            ...
            if self.data_type == "mc":
                ...
                img_LQ = util.read_img(
                    self.LQ_env, "{}.{}".format(name_a, v), LQ_size_tuple
                )
            else:
        ...
        if self.opt["phase"] == "train":
            ...
            if self.LR_input:
                ...
                img_GT = img_GT[
                    rnd_h_HR : rnd_h_HR + GT_size, rnd_w_HR : rnd_w_HR + GT_size, :
                ]
            else:
